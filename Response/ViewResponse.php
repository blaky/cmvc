...
class ViewResponse extends AbstractResponse {
     ...
     *
     * @param string $viewName the path of the view that will be used for rendering, it must be located in src/template/views folder
     * @param array:mixed $dataMap the data map for the view that will be put into the $m variable when rendering
     * @param string $frameName the name of the frame that the view will be rendered in. If set to null, the default frame will be used
     */
    ...
    public function build($requestParameters, $cfw) {
        ...
        $compiledComponents = $this->renderComponents(
                array_unique(array_merge($this->getComponentList(), $frameComponents), SORT_REGULAR), $requestParameters, $cfw);
        
        $compiledView = $this->renderView($compiledComponents, $requestParameters);
        
        $assembledPage = $this->renderFrame($frame, $compiledView, $compiledComponents, $requestParameters);
    ...
    private function renderComponents($components, $params, $cfw) {
        ...
        foreach ($components as $componentName) {
            ...
            $templatePath = $u->getTemplateDir() . "components/" . $component->getTemplateName($params) . ".php";
            
            if (file_exists($templatePath)) {
                ob_start();
                extract(
                        array(
                                'm' => $component->getDataMap($params), 
                                'gp' => $this->getGlobalParams()
                        ));
    ...
    private function renderView($compiledComponents, $params) {
        ...
        if (file_exists($templatePath)) {
            ...
            throw new \Exception("Template file was not found for '" . $this->viewName . "' component");
        }
    }
